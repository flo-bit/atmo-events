...
export const ATM_XRPC_METHODS = {
	...
	app: {
		...
		assertRecipient: 'money.atmosphere.app.assertRecipient',
	...
	tickets: {
		...
		previewTicketOrder: 'tickets.atmosphere.previewTicketOrder',
...
export type AtmSubscriptionActiveLimit = 'multiple' | 'one_per_payer_recipient';
...
export function inspectAtmAttestationRefs(
	...
	const payerKnown = Boolean(payerUri && payerDid && payerUri.startsWith(`at://${payerDid}/`));
...
/** One requested ticket tier and quantity, shared by paid holds and free claims. */
export type AtmTicketItemRequest = {
	tierId: string;
	quantity: number;
};
...
export type AtmTicketHoldInput = {
	...
	/** Optional organizer offer code; ATM validates and reserves it atomically with inventory. */
	offerCode?: string;
	items: AtmTicketItemRequest[];
	returnUrl?: string;
	cancelUrl?: string;
	idempotencyKey?: string;
	metadata?: Record<string, unknown>;
...
/** Non-reserving, app-authenticated ticket price preview. */
export type AtmTicketOrderPreviewInput = {
	environment?: AtmEnvironment;
	eventId?: string;
	eventUri?: string;
	tierId: string;
...
export type AtmTicketSummary = {
	...
	presentation?: AtmTicketPresentationSummary;
	[key: string]: unknown;
};
...
export type AtmTicketPresentationSummary = {
	iconUrl?: string;
	event?: {
		uri?: string;
		title?: string;
...
/**
 * Generic organizer handoff used by event apps. The app writes the canonical
 * event record first, then supplies its exact URI/CID here. ATM creates only a
 * paused sales shell; activation remains a separate, readiness-gated step.
 */
export type AtmStartTicketSetupInput = {
	environment: AtmEnvironment;
	organizerDid: string;
	/** Exact canonical public event strong reference written by the event app. */
	event: AtmTicketEventInput & { uri: string; cid: string; title: string };
	/** Resume target after app approval or payout onboarding. */
	approvalReturnUrl?: string;
	/** Return target after the organizer finishes ATM ticket configuration. */
	setupReturnUrl: string;
	requestReason?: string;
	feeShareBps?: number;
	publicRecords?: AtmPublicRecordsPolicy;
	metadata?: Record<string, unknown>;
	/** Mint organizer-scoped service-auth from the signed-in organizer's PDS. */
	getOrganizerAssertionToken: AtmServiceAuthTokenProvider;
};

export type AtmStartTicketSetupResult = {
	approval: AtmRequestRecipientApprovalResult;
	event: AtmTicketEventSummary;
	manageUrl: string;
	nextUrl: string;
};
...
export type AtmEventDataFor<TType extends AtmWebhookEventType> =
	TType extends keyof AtmEventDataByType ? AtmEventDataByType[TType] : Record<string, unknown>;

export type AtmTypedEvent<TType extends AtmWebhookEventType> = AtmWebhookEnvelope<
	AtmEventDataFor<TType>
> & { type: TType };

export type AtmTypedWebhookEnvelope<TType extends keyof AtmEventDataByType> = AtmWebhookEnvelope<
...
export type AtmFeeDistributionStatus = 'queued' | 'transferred' | 'held' | 'reversed' | 'failed';
...
export type AtmTicketOrderPreviewResult = {
...
export type AtmWebhookHandlerEvent<TType extends AtmWebhookEventType | undefined = undefined> =
	TType extends AtmWebhookEventType ? AtmTypedEvent<TType> : AtmWebhookEnvelope;
...
export type AtmWebhookHandlerOptions<TType extends AtmWebhookEventType | undefined = undefined> = {
	secret: string | readonly string[];
	expectedType?: TType;
	toleranceSeconds?: number;
	now?: number;
...
export type AtmWebhookDeliveryStore<TType extends AtmWebhookEventType | undefined = undefined> = {
	claim: (
		deliveryId: string,
		event: AtmWebhookHandlerEvent<TType>
...
export function createAtmCheckoutProduct(input: AtmCheckoutEnvelope): string {
	...
	const { customerEmail: _ignoredCustomerEmail, ...supportedInput } =
		input as AtmCheckoutEnvelope & { customerEmail?: unknown };
...
export function createFreeTicketClaimBody(input: AtmFreeTicketClaimInput): AtmFreeTicketClaimInput {
	assertDid(input.buyerDid, 'buyerDid');
	if (!input.buyerAssertionJwt) {
		throw new Error('free ticket claims require buyerAssertionJwt');
	}
	if (!Array.isArray(input.items) || input.items.length === 0) {
		throw new Error('free ticket claims require at least one item');
	}
	for (const item of input.items) {
		if (!item.tierId) {
			throw new Error('free ticket claim items require tierId');
		}
		if (!Number.isInteger(item.quantity) || item.quantity < 1) {
			throw new Error('free ticket claim item quantity must be a positive integer');
		}
	}
	return pruneUndefined(input);
}
...
export function signAtmWebhookPayload(opts: {
	...
	const digest = createHmac('sha256', opts.secret).update(signedPayload).digest('hex');
	return `t=${timestamp},v1=${digest}`;
}

export function verifyAtmWebhookSignature(opts: VerifyAtmWebhookOptions): boolean {
...
export function createNodeWebhookHandler<TType extends AtmWebhookEventType | undefined = undefined>(
...
export function createHonoWebhookHandler<TType extends AtmWebhookEventType | undefined = undefined>(
	options: AtmWebhookHandlerOptions<TType>
) {
	...
	return function handleAtmHonoWebhook(
		...
		const request =
			contextOrRequest instanceof Request ? contextOrRequest : contextOrRequest.req.raw;
...
export function createCloudflareWorkerWebhookHandler<
	TType extends AtmWebhookEventType | undefined = undefined
>(options: AtmWebhookHandlerOptions<TType>): AtmCloudflareWorkerWebhookHandler {
	const handler = createNodeWebhookHandler(options);
	return {
...
>(options: AtmExpressWebhookHandlerOptions<TType>) {
	return async function handleAtmExpressWebhook(
		...
		try {
			...
			if (options.onError) {
				const normalized = normalizePlainWebhookErrorHandlerResult(await options.onError(error));
				sendExpressJson(response, normalized.status, normalized.body, normalized.headers);
				return;
			}
			if (next) {
...
export async function constructAtmXrpcReceiverEvent<TData = Record<string, unknown>>(
...
export async function constructTypedAtmXrpcReceiverEvent<TType extends AtmWebhookEventType>(
...
export function createAtmAppClient(options: AtmAppClientOptions) {
	...
	return {
		...
		getPayoutStatus(actor: string) {
			return callQuery<AtmPayoutStatus>(brokerUrl, ATM_XRPC_METHODS.actor.getPayoutStatus, {
				actor
			});
		},
		...
		updateAppConfig(input: AtmUpdateAppConfigInput) {
			return callJson<AtmAppConfig>(brokerUrl, 'POST', ATM_XRPC_METHODS.app.updateConfig, input);
		},
		...
		/**
		 * Start an event-app → ATM organizer setup handoff against an event record
		 * the app has already written. The ATM shell is always created paused.
		 */
		async startTicketSetup(input: AtmStartTicketSetupInput) {
			const recipientAssertionJwt = await input.getOrganizerAssertionToken({
				lxm: ATM_XRPC_METHODS.app.assertRecipient,
				aud: audience
			});
			const approval = await callJson<AtmRequestRecipientApprovalResult>(
				brokerUrl,
				'POST',
				ATM_XRPC_METHODS.app.requestRecipientApproval,
				{
					recipientDid: input.organizerDid,
					environment: input.environment,
					paymentTypes: ['ticket'],
					...(input.feeShareBps !== undefined ? { feeShareBps: input.feeShareBps } : {}),
					...(input.publicRecords ? { publicRecords: input.publicRecords } : {}),
					requestReason: input.requestReason ?? `Set up ticket sales for ${input.event.title}`,
					setupReturnUrl: input.approvalReturnUrl ?? input.setupReturnUrl,
					recipientAssertionJwt
				}
			);
			const organizerAssertionJwt = await input.getOrganizerAssertionToken({
				lxm: ATM_XRPC_METHODS.tickets.createTicketEvent,
				aud: audience
			});
			const created = await callJson<AtmTicketEventResult>(
				brokerUrl,
				'POST',
			...
			if (!event || typeof event.id !== 'string' || event.id.trim().length === 0) {
				throw new AtmApiError(
					'InvalidTicketSetupResponse',
					'ATM returned an invalid ticket-event setup response.',
					502,
					created
				);
			}
			const manage = new URL(approval.dashboardUrl);
			manage.searchParams.set('tab', 'earn');
			manage.searchParams.set('section', 'tickets');
			manage.searchParams.set('earnContext', 'creator');
			manage.searchParams.set('appEnv', input.environment);
			manage.searchParams.set('ticketEvent', event.id);
			manage.searchParams.set('ticketSetupReturn', input.setupReturnUrl);
			const manageUrl = manage.toString();
			const nextUrl =
				approval.status !== 'approved'
					? approval.dashboardUrl
					: approval.needsPaymentSetup
						? approval.onboardingUrl
						: manageUrl;
			return {
				approval,
				event,
				manageUrl,
				nextUrl
			} satisfies AtmStartTicketSetupResult;
		},
		...
		getPaymentStatus(token: string) {
			return callQuery<AtmPaymentStatus>(brokerUrl, ATM_XRPC_METHODS.payment.status, { token });
		},
		...
		previewTicketOrder(input: AtmTicketOrderPreviewInput) {
			return callJson<AtmTicketOrderPreviewResult>(
				brokerUrl,
				'POST',
				ATM_XRPC_METHODS.tickets.previewTicketOrder,
		...
		getProfile(actor: string) {
			return callQuery<AtmProfile>(appViewUrl, ATM_XRPC_METHODS.actor.getProfile, { actor });
		}
	};
}
...
async function processWebhookDelivery<TType extends AtmWebhookEventType | undefined>(
	options: AtmWebhookHandlerOptions<TType>,
	event: AtmWebhookHandlerEvent<TType>,
	context: { rawBody: string; request?: Request }
...
function constructWebhookHandlerEvent<TType extends AtmWebhookEventType | undefined>(
	rawBody: string,
	headers: AtmWebhookHeaders,
	options: AtmWebhookHandlerOptions<TType>
...
function expressHeaders(headers: AtmExpressLikeRequest['headers']): AtmWebhookHeaders {
	return {
...
function normalizeWebhookHandlerResult(result: AtmWebhookHandlerResult): Response {
...
function normalizeWebhookErrorHandlerResult(result: AtmWebhookHandlerResult): Response {
...
function normalizePlainWebhookHandlerResult(result: AtmWebhookHandlerResult): {
	status: number;
	body: unknown;
	headers?: Record<string, string>;
} {
...
function normalizePlainWebhookErrorHandlerResult(result: AtmWebhookHandlerResult): {
	status: number;
	body: unknown;
	headers?: Record<string, string>;
} {
...
function defaultPlainWebhookError(error: unknown): {
	status: number;
	body: unknown;
	headers?: Record<string, string>;
} {
