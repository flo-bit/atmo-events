<script lang="ts">
	...
	function populateLocationFromEventData() {
		...
		if (eventData.locations && eventData.locations.length > 0) {
			const loc = eventData.locations.find((v) => v.$type === 'community.lexicon.location.address') as
				| { street?: string; locality?: string; region?: string; country?: string }
				| undefined;
			if (loc) {
	...
	async function handleSubmit() {
		...
		try {
			...
			const createdAt = isNew
				...
				: data.eventData?.createdAt || new Date().toISOString();
